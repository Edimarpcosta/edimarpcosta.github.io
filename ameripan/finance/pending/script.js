...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function inicializarApp() {
    ...
    if (syncStartDateInput && syncEndDateInput) {
      ...
      syncStartDateInput.value = formatDateForInput(startOfMonth);
      syncEndDateInput.value = formatDateForInput(endOfMonth);
    }
  ...
  function renderClientsTable() {
    ...
    const filtered = clientsData.filter(c => {
      const matchesQuery = c.Cliente.toLowerCase().includes(query) || 
                           c.Parcela.toLowerCase().includes(query) ||
                           c.Lanco.includes(query) ||
                           c.NF.includes(query);
    ...
    filtered.forEach((c) => {
      const realIndex = clientsData.findIndex(orig => orig.ChaveUnica === c.ChaveUnica);
      const isSelected = selectedClientIndex === realIndex;
      ...
      tr.innerHTML = `
        <td class="font-semibold">
          <div><span class="text-cyan-400 font-mono font-bold mr-1.5">[${c.Codigo || '--'}]</span>${c.Cliente}</div>
          <div class="text-[9px] text-slate-555 font-mono mt-0.5">Lanço: ${c.Lanco} | NF: ${c.NF || '--'}</div>
        </td>
      ...
      tr.addEventListener("click", () => {
        const oldSelected = clientsTableBody.querySelector(".selected");
  ...
  function inspecionarCliente(index) {
    ...
    // Encontrar todas as faturas do cliente que estão pendentes (não pagas)
    const faturasExibidas = clientsData.filter(c => {
      if (client.CNPJ && c.CNPJ === client.CNPJ) return c.Status !== "Pago";
      if (client.Codigo && c.Codigo === client.Codigo) return c.Status !== "Pago";
      return c.Cliente === client.Cliente && c.Status !== "Pago";
    });

    // Garante que a fatura clicada esteja na lista (mesmo se estiver paga)
    if (!faturasExibidas.some(f => f.ChaveUnica === client.ChaveUnica)) {
      faturasExibidas.push(client);
    }
        ...
        <div class="space-y-3">
          ...
          <div class="inspector-faturas-box space-y-1.5 p-2.5 rounded-xl">
            ...
            <div class="flex justify-between items-center pt-1.5 border-t border-slate-900 mt-1 text-xs">
              <span class="font-bold text-slate-400">Total Selecionado:</span>
    ...
    sendWhatsappBtn.addEventListener("click", () => {
      ...
      // Encontrar todas as faturas selecionadas (não pagas) para atualizar
      const checks = inspectorContent.querySelectorAll(".fatura-check");
      const faturasSelecionadas = [];
      checks.forEach(check => {
        if (check.checked) {
    ...
    saveInspectorBtn.addEventListener("click", () => {
      ...
      // Obter contatos corrigidos digitados
      ...
      // Sinalização de telefones confirmados
      ...
      // Atualizar no mapa de contatos corrigidos por Código (com fallback a CNPJ)
      const clientKey = String(client.Codigo || client.CNPJ || "").trim();
      if (clientKey) {
        contatosCorrigidosMap[clientKey] = {
      ...
      // Obter marcadores comportamentais
      ...
      // Obter todas as faturas marcadas para salvar status
      const checks = inspectorContent.querySelectorAll(".fatura-check");
      const faturasSelecionadas = [];
      checks.forEach(check => {
        if (check.checked) {
